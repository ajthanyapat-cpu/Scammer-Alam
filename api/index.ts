...
app.post(['/api/analyze-number', '/analyze-number'], async (req: Request, res: Response) => {
...
ส่งผลลัพธ์กลับมาในรูปแบบ JSON ตาม Schema นี้เท่านั้น:
{
  "phoneNumber": "${phoneNumber}",
  "riskScore": number (0 ถึง 100),
  "riskLevel": "extreme" | "high" | "medium" | "safe",
  "summary": "สรุปสั้นกระชับเตือนภัยภาษาไทย",
  "identifiedTactics": ["กลโกง/จิตวิทยาที่ตรวจพบ"],
  "dangerIndicators": ["จุดอันตราย"],
  "recommendedActions": ["สิ่งที่เหยื่อต้องทำทันที"],
...
app.get(['/api/community-reports', '/community-reports'], (req: Request, res: Response) => {
  res.json({ reports: communityReportsCache });
});

app.post(['/api/community-reports', '/community-reports'], (req: Request, res: Response) => {
...
app.get(['/api/health', '/health'], (req: Request, res: Response) => {
  res.json({ status: 'ok', service: 'ScamShield Anti-Fraud API (Vercel Serverless Ready)' });
});
